...
/** A type for a vector of unique pointers to map database objects. For internal use only. */
...
class CFrameworkMapDataSet
    ...
    TResult DeleteMapObjectArray(uint32_t aMapHandle,const uint64_t* aIdArray,size_t aIdCount,uint64& aDeletedCount,CString aCondition = nullptr);
...
class CFramework: public MNavigatorObserver
    ...
    */
    ...
    bool ObjectWouldBeDrawn(TResult& aError,uint64_t aId,TMapObjectType aType,const CString& aLayer,uint32_t aIntAttrib,const CString& aStringAttrib) const;
    ...
    uint32_t SetNearbyObjectWarning(uint32_t aId,const CString& aLayer,const CString& aCondition,double aMaxDistance,size_t aMaxObjectCount);
    uint32_t SetVehicleTypeWarning(double aMaxDistance,size_t aMaxObjectCount);
    ...
    double PolygonArea(const TCoordSet& aCoordSet,TCoordType aCoordType) const;
    double PolylineLength(const TCoordSet& aCoordSet,TCoordType aCoordType) const;
    void GetAreaAndLength(const CGeometry& aGeometry,double& aArea,double& aLength) const;
...
class CMapRenderer
    ...
    using a separate thread which calls Draw() 25 times per second. This feature is supported on Windows only.
    */
