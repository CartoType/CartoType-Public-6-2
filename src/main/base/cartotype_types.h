...
#ifndef CARTOTYPE_FORCE_STANDARD_INTEGER_NAMES

/** An 8-bit signed integer. Obsolete: use int8_t. */
using int8 = int8_t;
/** An 8-bit unsigned integer. Obsolete: use uint8_t. */
using uint8 = uint8_t;
/** A 16-bit signed integer. Obsolete: use int16_t. */
using int16 = int16_t;
/** A 16-bit unsigned integer. Obsolete: use uint16_t. */
using uint16 = uint16_t;
/** A 32-bit signed integer. Obsolete: use int32_t. */
using int32 = int32_t;
/** A 32-bit unsigned integer. Obsolete: use uint32_t. */
using uint32 = uint32_t;
/** A 64-bit signed integer. Obsolete: use int64_t. */
using int64 = int64_t;
/** A 64-bit unsigned integer. Obsolete: use uint64_t. */
...
/**
This CartoType-specific version of DBL_MAX is supplied to avoid the bug in
C++ Builder, which defines DBL_MAX as a very small number.
It uses a different name from DBL_MAX in case DBL_MAX is defined as a macro.
*/
