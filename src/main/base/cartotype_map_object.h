...
class CMapObject: public MPath
    ...
    */
    ...
    /**
    Splits a map object, if possible, into objects with no more than aMaxPoints points each. Puts
    any extra objects created into aExtraObjectArray.
    The base implementation does nothing.
    */
    ...
    */
    ...
    /**
    Returns the center of a map object, defined
    as the center of gravity for polygons and arrays, half way along the path for lines,
    ...
    */
    ...
    meet without gaps.
    */
    std::unique_ptr<CMapObject> Interpolate(int32 aGranularity) const;
    ...
    if aProjectionFromLatLong is non-null, writes the data as degrees lat/long,
    using the inverse of the projection to convert the points, otherwise
    writes the data as is.
    */
    TResult Write(TDataOutputStream& aOutput,const CProjection* aProjectionFromLatLong = nullptr) const;
    /**
    Calculates the area of a map object in square metres.
    Returns 0 if the object is a point or line object.

    aProjection must be the projection used to project this object from lat/long.
    */
    double Area(TResult& aError,const CProjection& aProjection) const;
    /**
    Calculates the length or perimeter of a map object in metres.
    ...
    */
    ...
    ignoring case and accents, it's a full match or a fuzzy match.

    If it matches part of the attribute it's a partial match.
    */
