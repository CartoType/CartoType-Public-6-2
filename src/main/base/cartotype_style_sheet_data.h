...
class CStyleSheetData
    ...
    /** Creates a CStyleSheetData object representing data stored in memory. Copies the data. Does not check for aData being null or aLength being zero. */
    ...
    /** Creates a CStyleSheetData object representing data stored in memory. Copies the data. Returns KErrorInvalidArgument if aData is null or aLength is zero. */
    CStyleSheetData(TResult& aError,const uint8_t* aData,size_t aLength);
    /** Reloads the data in a CStyleSheetData object from the file if any. If no filename is stored, returns KErrorUnimplemented. */
    TResult Reload();
