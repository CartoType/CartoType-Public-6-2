...
class MPath
    ...
    */
    ...
    COutline TransformedPath(TResult& aError,MPointTransformer& aTransformer,int32_t aFractionalBits) const;
...
class TContour: public MContour
    ...
    class Iter
        ...
        const TOutlinePoint& operator*() const { if (m_has_off_curve_points) m_point = *((TOutlinePoint*)m_ptr); else m_point = *m_ptr; return m_point; }

        private:
        const TPoint* m_ptr = nullptr;
        mutable TOutlinePoint m_point;
    ...
    class LineIter
        ...
        LineIter(const TContour& aContour,size_t aIndex):
            ...
            if (m_index == 0)
                ...
                m_line.iStart = m_contour.LastPoint();
                m_line.iEnd = m_contour.Point(0);
                if (!m_contour.Closed() || m_line.iStart == m_line.iEnd)
                    ++(*this);
                }
            }
        /** Increments this iterator to refer to the next line. */
        LineIter& operator++() { if (++m_index < m_contour.Points()) { m_line.iStart = m_line.iEnd; m_line.iEnd = m_contour.Point(m_index); } return *this; }
        /** The inequality operator. */
...
class TSimpleContourData: public MWritableContour
    ...
    void SetPoint(size_t aIndex,const TOutlinePoint& aPoint) override { assert(aIndex < iPoints); assert(aPoint.iType == TPointType::OnCurve); iPoint[aIndex] = { aPoint }; }
...
class COnCurveContour: public CContourBase<TPoint>
    ...
    void AppendPoint(const TOutlinePoint& aPoint) override
        {
        if (iPoint.size() && aPoint == iPoint.back())
            return;
        iPoint.push_back(aPoint);
        }
    /** Appends a point to the contour whether or not it differs from the previous point. */
