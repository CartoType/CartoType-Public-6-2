...
class TImageData
    {
    public:
    /** The bitmap if any. Not owned. */
    const TBitmap* iBitmap = nullptr;
    /** Arbitrary data in a format known to the user. Not owned. */
    void* iImageData = nullptr;
    /** The size of the image data stored in iImageData in bytes. */
...
class MImageServerHelper
    ...
    */
    virtual TResult DrawBitmap(void* aDeviceContext,const TBitmap* aBitmap,const void* aImageData,const TTransform& aTransform) = 0;
    };
