...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState)
        ...
        m_location_callback = new LocationCallback()
            {
            @Override
            public void onLocationResult(LocationResult aResult)
                {
                if (aResult == null)
                    return;
    ...
    void chooseAddress(Address aAddress)
        {
    ...
    void startNavigating()
        ...
        confirm("NAVIGATION IS FOR TESTING ONLY AND NOT INTENDED FOR ACTUAL ROUTE GUIDANCE", "Press OK to confirm.", new Function<Void,Void>()
            ...
            public Void apply(Void input)
                {
                m_location_request = new LocationRequest();
                m_location_request.setInterval(1000);
    ...
    private FusedLocationProviderClient m_location_provider_client;
